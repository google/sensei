...
#ifndef SENSEI_UTIL_GTL_MAP_UTIL_H_
#define SENSEI_UTIL_GTL_MAP_UTIL_H_
...
template <typename P>
struct PointeeTypeImpl { typedef typename P::element_type type; };
template <typename T>
struct PointeeTypeImpl<T*> { typedef T type; };
template <typename P> struct PointeeType : PointeeTypeImpl<P> {};
template <typename P> struct PointeeType<const P> : PointeeType<P> {};
template <typename P> struct PointeeType<P&> : PointeeType<P> {};
...
// expression instead. Client code has the advantage of access to C++11. E.g.:
// - Old
//   auto *raw = FindLinkedPtrOrNull(collection, key);
// - Alternative 1 (simple syntax)
//   auto *raw = FindWithDefault(collection, key, {}).get();
// - Alternative 2 (more flexibility in some cases)
//   auto it = collection.find(key);
//   auto *raw = it == collection.end() ? nullptr : it->second.get();
template <class Collection>
...
LookupOrInsertNew(Collection* const collection,
  ...
  typedef typename util::gtl::map_util_internal::PointeeType<Mapped>::type
      Element;
  std::pair<typename Collection::iterator, bool> ret =
...
ReverseCollection ReverseMap(const Collection& collection) {
  typename util::gtl::map_util_internal::RemoveConst<ReverseCollection>::type
      reverse;
...
// Erases all elements of container where predicate evaluates to true.
template <class MapContainer, class Predicate>
void AssociativeEraseIf(MapContainer* container, Predicate predicate) {
  CHECK(container != NULL);
  typedef typename MapContainer::iterator Iter;
...
#endif  // SENSEI_UTIL_GTL_MAP_UTIL_H_
