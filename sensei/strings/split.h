...
// splitting API.
//
#ifndef SENSEI_STRINGS_SPLIT_H_
#define SENSEI_STRINGS_SPLIT_H_
...
#include "sensei/strings/charset.h"
#include "sensei/strings/split_internal.h"
#include "sensei/strings/stringpiece.h"
#include "sensei/strings/strip.h"
...
//   // vector of strings.
//   vector<string> v = strings::Split("a,b,c", ",");  // Could also use ','
//   assert(v.size() == 3);
//
// Example 2:
...
//
//   - Literal (default for string arguments)
//   - SingleChar (default for char argument)
//   - AnyOf
//   - Limit
//   - FixedLength
//
...
//
//   - //i18n/utf8:strutil provides delimiters for utf8-oriented splitting.
//
...
//
// Example 2:
//   // Because a char literal is converted to a strings::delimiter::SingleChar,
//   // the following two splits are equivalent.
//   vector<string> v1 = strings::Split("a,b,c", ',');              // (1)
//   using ::strings::delimiter::SingleChar;
//   vector<string> v2 = strings::Split("a,b,c", SingleChar(','));  // (2)
//
// Example 3:
...
//   // matching at most one time. The last element in the returned collection
...
// Represents a literal string delimiter. The delimiter string is copied and
// owned by this class. Examples:
//
//   using ::strings::delimiter::Literal;
...
// Represents a single character delimiter.
//
//   using ::strings::delimiter::SingleChar;
//   vector<string> v = strings::Split("a-b-c", SingleChar('-'));
//   assert(v.size() == 3);
//   assert(v[0] == "a");
//   assert(v[1] == "b");
//   assert(v[2] == "c");
//
// This is also the *default* delimiter used if a single character is given as
// the delimiter to the strings::Split() function. For example, the following
// calls are equivalent.
//
//   vector<string> v = strings::Split("a-b", '-');
//
//   using ::strings::delimiter::SingleChar;
//   vector<string> v = strings::Split("a-b", SingleChar('-'));
//
class SingleChar {
 public:
  explicit SingleChar(char c);
  StringPiece Find(StringPiece text, size_t pos) const;
...
template <> struct SelectDelimiter<char>        { typedef SingleChar Type; };
...
//        &safe_strto64, &values));
//  CHECK_EQ(3, values.size());  // set<> retains only unique values
//
//  deque<int64> values;
...
bool SplitStringIntoKeyValues(const string& line,
                              ...
                              string* key, std::vector<string>* values);
...
const char* SplitLeadingDec64Values(const char* next,
