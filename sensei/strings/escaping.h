...
#ifndef SENSEI_STRINGS_ESCAPING_H_
#define SENSEI_STRINGS_ESCAPING_H_
...
//
//        string encoded =
//            StrCat(strings::BackslashEscape(prefix, delims), d, suffix);
//        size_t end = strings::BackslashUnescapedFind(encoded, delims);
//        string decoded = strings::BackslashUnescape(
//            encoded.substr(0,end), delims);
//
...
//    Check out http://tools.ietf.org/html/rfc2045 for more details,
...
//    where the breaks occurred. If "src" contains any embedded null characters,
//    processing will stop when the null character is encountered.
//
...
//
//    Rewrites quoted printable escape sequences =XX in src to their
//    single-byte equivalents and returns the rewritten string.
// ----------------------------------------------------------------------
string QuotedPrintableUnescape(StringPiece src);

// ----------------------------------------------------------------------
// QEncodingUnescape()
//    This is very similar to QuotedPrintableUnescape except that we convert
//    '_'s into spaces. See http://tools.ietf.org/html/rfc2047
//
...
//    is encountered.
// ----------------------------------------------------------------------
string QEncodingUnescape(StringPiece src);
...
//    binary equivalents. src is not null terminated, instead specify len.
//    RETURNS the length of dest, or -1 if src contains invalid chars.
// ----------------------------------------------------------------------
...
//   whole base32 quanta.
//
//   See RFC3548 at http://tools.ietf.org/html/rfc3548
...
// A strings::Join()-compatible formatter that invokes the above ShellEscape()
// function on each element being joined.
//
// Usage example:
//   vector<string> argv = ...;
//   string cmdline = strings::Join(argv, " ", ShellEscapeFormatter());
//
struct ShellEscapeFormatter {
  void operator()(string* out, StringPiece sp) const {
    out->append(ShellEscape(sp));
  }
};
...
string ShellEscapeCommandLine(InputIterator begin, const InputIterator& end) {
  return strings::Join(begin, end, " ", ShellEscapeFormatter());
}
...
#endif  // SENSEI_STRINGS_ESCAPING_H_
